...
const relayRequest = {
    ...
    metadata:{
     ...
     signature: '0xd1fedb3e4065e565f08653be4a9722d4ee65575aca96e9d199c749536ee189f810b893e6ab957cf4d56decec7bdac198c486ccf1c4377f0797689fd8d64147911c',
     relayMaxNonce: 3
    }
 };
...
describe('Relay Server Test', () => {
    it('relay should return signedTx and transactionHash', async () => {
        ...
        const managerKeyManagerStub : SinonStubbedInstance<KeyManager> = createStubInstance(KeyManager);
        const workersKeyManagerStub : SinonStubbedInstance<KeyManager> = createStubInstance(KeyManager);
        const contractInteractorStub : SinonStubbedInstance<ContractInteractor> = createStubInstance(ContractInteractor);
        const txStoreManagerStub : SinonStubbedInstance<TxStoreManager> = createStubInstance(TxStoreManager);
        const IRelayHubInstanceStub : any = {};
        const transactionManagerStub : SinonStubbedInstance<TransactionManager> = createStubInstance(TransactionManager);
        ...
        workersKeyManagerStub.getAddress.returns(relayRequest.relayRequest.relayData.relayWorker);
        transactionManagerStub.sendTransaction.resolves({...transactionTestResponse});
        ...
        assert.equal(txInfo.transactionHash,transactionTestResponse.transactionHash);
